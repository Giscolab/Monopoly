...
| `Source/monopoly/Ai_trade.cpp` | `AITradeUtility`, `AIUtility`, `BoardRules` | `PORTED_PARTIAL` | recherche recursive du plus petit groupe de trade de monopole | noyau AI pur; futurs evaluation/negociation et RuleTrade | `AI_Find_Smallest_Monopoly_Trade` est porte dans `AITradeUtility` avec la recursion retail, la condition de monopoles suffisants pour tous les participants, la reduction aux sous-ensembles et des gardes modernes sur les indices joueurs. `AI_Get_Transfer_Tax`, `AI_Num_Veto_Monopolies` et `AI_Is_Possible_Monopoly` sont aussi portes/testes, y compris taxe uniquement sur biens hypotheses, omission retail du groupe brun dans le veto-count et scan sparse railroad/utility. `AI_Should_Trade_For_Monopoly` est porte avec les decisions FALSE/TRUE/AI_MAYBE, le stade Buying, les monopoles existants et le seuil de retard de revenu de 20 %. Le bug source qui reutilise `current_player` apres la boucle (donc `NumberOfPlayers`, hors plage) est corrige en comparant/indexant le `player` demande, avec regressions leader/retard. Le reste de `Ai_trade.cpp` (valorisation, construction/counter-offers, attitudes, immunites/futures et envoi) reste a porter. |
| `Source/monopoly/Ai_util.cpp` | `AIUtility`, `BoardRules` | `PORTED_PARTIAL` | groupes/sets de proprietes, monopoles, lots, loyers, proximite, revenus/depenses, highest-rent/best-current-rent, stade de partie, maisons/hotels, actifs liquides, valeur totale | `GameState`, `BoardRules`; futurs `AI_State`/`Ai_load` | `AIUtility` porte maintenant le noyau pur de base de `Ai_util.cpp` sans etat global AI : plages `AI_Group_Start_End`, sets/comptage, detection/proprietaire de monopoles, lots, maisons/hotels et shortage, railroads/utilities, `AI_Rent_If_Stepped_On`, table retail des frequences d atterrissage, proximite joueur/monopole, cout d unmortgage a 110 %, cash cows, revenu potentiel, rente moyenne recue/payee, depenses minimales, highest-rent/best-current-rent, `AI_Get_Monopolies`, comptage des 22 lots couleur encore a acheter et `AI_Get_Monopoly_Stage`, actifs liquides de proprietes et total worth. Les quirks retail documentes (railroads dans `AI_Is_Monopoly`, exclusion dans `AI_Own_Monopoly`, semantique mortgage, inversion historique `count_baltic`, calcul fiscal utilise par `AI_Average_Rent_Payed`) sont preserves/testes. Trois bugs sources explicites sont corriges/testes : affectation inversee de `AI_Highest_Rent_Square`, mauvais property-set employe pour le loyer adverse, et `AI_Best_Current_Rent` classe sur le loyer recu plutot que `AI_Average_Rent_Payed`. Le port couvre aussi `AI_Test_Sell_Houses`, le choix de mortgage a impact de rente minimal et le meilleur unmortgage sous plafond, y compris la decomposition d hotel sous penurie et le bord flottant retail de `mortgageCost * 1.1`. `AI_Square_Move_Reaction` est aussi porte : seuils Happy/Sad/Angry, cartes Chance/Community speciales, couts de reparation, mouvements recursifs et reaction prison. Deux bugs sources supplementaires sont corriges/testes : loyer adverse calcule avec le property-set du proprietaire (pas du payeur) et comparaison de l exposition prison au seuil dollar 500 (pas a l enum `AI_REACTION_ANGRY`=3). Les helpers de preparation au trading `AI_Num_Of_Monopolies_In_Set`, `AI_Num_Of_Monopolies_Between_Players`, `AI_Remove_Players_From_List` et `AI_XOR_Property_Set` sont aussi portes/testes. Restent notamment `AI_State.money_owed`, hypothese achat/unmortgage complete, heuristiques chargees depuis `*.AI`, trading decisionnel et boucle de decision. |
...
Le 2026-09-08, avec MSVC 19.51 / toolset 14.51, Visual Studio 18 2026,
...
- suite complete depuis ce build neuf : **83/83 suites passees**, zero echec ;
...
1. Continuer `UDOpts` dans l ordre source : l entree IBar, l ecran File autonome et la navigation File/Option/Credits/Help sont fermes; porter ensuite le cadre du sous-ecran Option (titre/sous-titres/OK), puis uniquement les toggles ayant un owner moderne reel, sans inventer les surfaces texte/fonts ni les confirmations Load/Save/Exit.
2. `UDTrade` est ferme pour toutes les sequences DAT autonomes actives; conserver `rightpanel`/listes/noms/cash texte explicitement differes jusqu au backend GRAFIX/FONTS portable. `UDAuct` reste ferme hors surfaces texte et audio/voix Pennybags.
3. Auditer les autres types HMD effectivement atteignables sous `USE_OLD_FRAME`; ne porter qu un type prouve par caller/source et laisser reset/joint MIMe ou primitives non consommees explicitement non supportes.
