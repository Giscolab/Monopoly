...
| `Source/monopoly/Ai_util.cpp` | `AIUtility`, `BoardRules` | `PORTED_PARTIAL` | groupes/sets de proprietes, monopoles, lots, loyers, maisons/hotels, actifs liquides, valeur totale | `GameState`, `BoardRules`; futurs `AI_State`/`Ai_load` | `AIUtility` porte un premier noyau pur de `Ai_util.cpp` sans etat global AI : plages `AI_Group_Start_End`, sets et comptage, detection/proprietaire de monopoles, lots avec le plus cher en tete, comptage/capacite maisons, inventaires houses/hotels et shortage, railroads/utilities, calcul `AI_Rent_If_Stepped_On`, actifs liquides de proprietes et total worth. Les quirks retail (railroads acceptes par `AI_Is_Monopoly`, exclusion dans `AI_Own_Monopoly`, semantique mortgage et inversion historique `count_baltic`) sont preserves/testes. Restent notamment `AI_State.money_owed`, revenus/depenses moyens, heuristiques chargees depuis `*.AI`, trading et boucle de decision. |
