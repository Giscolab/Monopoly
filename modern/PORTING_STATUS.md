...
1. Continuer `UDBoard` dans l ordre source apres backdrops multi-ville, mesh 3D ville, ownership/mortgage et cameras : traiter ensuite le contrat `LIGHTING` seulement avec un vrai backend World3D (ambient 0.53/0.84, deux directionnelles, spotlight mobile/couleur joueur); ne pas simuler les appels `LE_REND3D_SetLight*`, et conserver le chemin custom 2D `city<0` differe tant que `g_aid2DBoards`/payloads externes ne sont pas disponibles.
