...
| `Source/monopoly/UDOpts.cpp` | `OptionsUI`, `OptionsFilePlayback`, `UserInterface`, `IBar`, `IBarBackdropPlayback` | `PORTED_PARTIAL` | entree Options, ecran File, cinq boutons, retour Cancel | DISPLAY, IBar, SequencePlayback, persistence/options dialogs, fonts | Le bouton global Options est maintenant r̩ellement expos̩ par le masque runtime quand une partie est en cours, m̩morise la vue IBar precedente et ouvre `Screen2D::Options`. `OptionsUI` porte l etat FILE..LOAD_GAME et les hotspots retail; `OptionsFilePlayback` rend `CNK_syfiles` et les cinq boutons New/Load/Save/Exit/Cancel via leurs CNK incoming DAT_LANG2, priorit̩ 50 et `StayAtEnd`, avec ouverture/fermeture transactionnelles. Cancel retourne exactement a Main/Trade/Portfolio. New/Load/Save/Exit sont identifi̩s sans simuler leurs confirmations/dialogues non portes. |
...
1. Continuer `UDOpts` dans l ordre source : l entree IBar et l ecran File autonome sont fermes; porter ensuite les sous-ecrans/options et dialogues uniquement quand leurs assets/callers sont prouv̩s, sans inventer les surfaces texte/fonts ni les confirmations Load/Save/Exit.
