...
| `Source/monopoly/UDPieces.cpp` | `PiecePlacement`, `PieceRuntime`, `PieceCamera`, `PieceMovePlan`, `BoardGeometry` | `PORTED_PARTIAL` | orientation tokens/repos, maisons/hotels, pose runtime, choix cameras et planification pure de TokenAnimStack | `SequenceRuntime`, TextureCatalog, PC3D moderne, BoardGeometry, RULE | Offsets 5x6, rotations token, constantes batiments, GetInfo/GetChildMeshWorldMatrix -> LastKnownData, mappings camera 3/5/15, `PickAGoodCamera` et `PlanMoveAnim` pur sont portes/testes. Les branches backwards, jail, off-board bankrupt/victory, tags DAT_3D, limite 12 et choix rand()%2 injectes sont couverts. Reste l execution runtime de la stack, show/lifecycle et raccordement complet au DISPLAY. |
...
- reconstruction **clean-first complete** de `modern/build-phase-c`, code de
  ...
  `MonopolyGPU3DCore` ;
- suite complete depuis ce build neuf : **38/38 suites passees**, zero echec ;
- trace conservee dans le build ignore sous
  `modern/build-phase-c/ctest-full-20260906-piece-move-plan.log` ;
