...
| `Source/monopoly/UDChat.cpp` | `ChatRuntime`, `Messaging`, `UserInterface` | `PORTED_PARTIAL` | historique texte, public/prive, fluff IDs, `ACTION_TEXT_CHAT` / `NOTIFY_TEXT_CHAT` | RULE echo, LocalPlayers, futur renderer/fonts | `ChatRuntime` porte le noyau runtime livre : historique circulaire 512 messages, texte 255 caracteres, cible publique/privee, encodage UTF-16LE et ouverture automatique a reception. RULE relaie deja le blob. Restent EditBox/input SDL, selection graphique des destinataires, word-wrap/fonts, surfaces GRAFIX, fluff UI et voice-chat. |
