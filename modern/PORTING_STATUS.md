...
| `Source/monopoly/UDIBar.cpp` | `IBar`, `IBarLayout`, `IBarBackdropPlayback`, `IBarPropertyPlayback`, `IBarBankPlayback`, `IBarCameraButtonPlayback`, `IBarCurrentPlayerPlayback`, `IBarRuleState` | `PORTED_PARTIAL` | initialize/destroy/tick/show/process, filtrage joueurs, backdrop, boutons globaux et d'etat, pion courant | LocalPlayers, DISPLAY, UserInterface, SequencePlayback, Overlay2D, assets/fonts | Overlay2D couvre backdrop joueur/banque, icone Banque, pion courant, boutons globaux, decisions tour/prison/taxe/achat, Bankrupt, BSSM Build/Sell/Mortgage/Unmort, AucHouse/AucHotel, PlaceHouse/PlaceHotel et TradeAcc/TradeCnt/TradeRej. Les playersets shortage et le suivi Trade A/B resolvent le joueur IBar sans heuristique. Les deux atlas `CNK_iyaaf`/`CNK_iycaf`, priorites 999/1000/1001/1002, `IBarIsStable`, `IBAR_JustChanged`, StartXYDrop et ordre d'index sont testes. `IBarLayout` porte aussi les neuf rectangles d'action 800x600 et les overrides BuyAuction/TaxDecision/Trading avec masque de slots actifs et bornes Win32 exclusives. `IBar::processLibraryMessage` filtre les clics par ce masque et route Buy/Auction, taxe, prison, trade, faillite et shortage vers RULE. Les sous-etats locaux Build/Sell/Mortgage/UnMortgage, `DeedActive`, les 28 positions de titres et leur playback DAT_MAIN `TAB_indsstc00`/`TAB_indsstg00`/`TAB_indsstm00` sont raccordes a Overlay2D avec selection de deed et actions rapides. Restent l'etat visuel Pressed, les blow-ups/mouseover de deeds, cartes, cash/textes et surfaces dynamiques. |
...
Le 2026-09-06, avec MSVC 19.51 / toolset 14.51, Visual Studio 18 2026,
...
- suite complete depuis ce build neuf : **64/64 suites passees**, zero echec ;
...
1. Continuer `UDIBar` apres le dispatch et `DeedActive` maintenant prouves : raccorder
   l'etat visuel `Pressed`/retour Idle des boutons puis les mouseovers/blow-ups de
   proprietes; ensuite porter les cartes Chance/Community Chest et messages
   cash/textes sans ouvrir prematurement le chantier fonts/surfaces dynamiques.
