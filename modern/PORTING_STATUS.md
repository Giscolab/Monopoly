...
1. Continuer `UDPsel` apres `CustomizeRules` dans l ordre source : auditer les phases/callers encore actifs et porter seulement les transactions/layouts deterministes; laisser les hotspots calcules par `LANG_GetStringWidth`/word-wrap au futur backend fonts/LANG. UDIBar reste borne aux consommateurs autonomes deja couverts et ne doit etre rouvert que lorsqu UDStats/UDTrade/UDOpts ou un backend fonts/surfaces debloquent un chemin reel.
