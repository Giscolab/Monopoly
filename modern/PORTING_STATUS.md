...
| `Source/monopoly/UDAuct.cpp` | `AuctionUI`, `AuctionPlayback`, `UserInterface`, `RuleAuction` | `PORTED_PARTIAL` | projection notifications, input billets, layout et playback statique de l ecran enchere | DISPLAY, LocalPlayers, SequencePlayback, RULE auction, fonts/audio | `AuctionUI` projette `NotifyNewHighBid`/`NotifyAuctionGoing`, conserve former view, third-going et roll-call, calcule les panneaux retail 201/134 et les sept billets 500/100/50/20/10/5/1, puis route `ACTION_BID` pour les humains locaux avant UDBoard comme `Userifce.cpp`. `AuctionPlayback` publie transactionnellement le bottom bar `TAB_anbg00`, deed `index+28*city` ou house/hotel, panneaux couleur, trays, bid plates et tokens aux priorites/positions retail; 4 joueurs donnent 18 objets statiques et 5 joueurs 22. Les surfaces texte nom/bid/cash/current-bid, le state machine graphique Pennybags et l audio restent a porter sans dupliquer l etat autoritatif de `RuleAuction`. |
...
- suite complete depuis ce build neuf : **74/74 suites passees**, zero echec ;
...
1. Continuer `UDAuct` dans l ordre source avec le state machine graphique Pennybags `CNK_an01..17`, la detection de fin de sequence, `BEGIN`/roll-call et `ENDAUCTION`/retour de vue. Garder audio et surfaces texte/fonts explicitement differes tant que leurs backends portables n existent pas.
2. Passer ensuite a `UDTrade`, puis `UDOpts`, en ne portant que des contrats actifs avec owner moderne clair; `UDPsel` et les contrats autonomes actifs de `UDPieces` sont fermes hors persistence/fonts/audio/options externes.
