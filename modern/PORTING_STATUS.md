...
- suite complete depuis ce build neuf : **72/72 suites passees**, zero echec ;
...
1. Continuer `UDPsel` apres `SelectPlayer` dans l ordre source : auditer d abord la persistance Player1..100 / HiScore, mais ne la raccorder que si un owner portable de preferences utilisateur est defini; sinon documenter la frontiere et passer au prochain caller actif. Laisser le rendu noms/HiScore et les hotspots calcules par `LANG_GetStringWidth`/word-wrap au futur backend fonts/LANG. UDIBar reste borne aux consommateurs autonomes deja couverts.
...
4. Poursuivre la Phase D en priorisant `UDAuct -> UDTrade -> UDOpts`; `UDPsel` et les contrats autonomes actifs de `UDPieces` sont maintenant fermes hors persistence/fonts/audio/options externes; revenir a UDIBar lorsque ces modules ou un backend fonts/surfaces debloquent ses derniers consommateurs, et reporter AI, audio/voice et video tant qu ils ne debloquent pas un chemin plus prioritaire.
