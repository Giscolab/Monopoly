...
| `Source/monopoly/UDPieces.cpp` | `PiecePlacement`, `PieceRuntime`, `PieceCamera`, `PieceMovePlan`, `PieceMovePlayback`, `PieceMoveIngress`, `PieceInterpolation`, `PieceJailPlan`, `PieceJailPlayback`, `PieceIdleTransition`, `PieceIdlePlayback`, `PieceIdleDisplay`, `PieceBuildingDisplay`, `BoardGeometry` | `PORTED_PARTIAL` | orientation tokens/repos, maisons/hotels, pose runtime, cameras, TokenAnimStack, game-queue movement ingress, GoToJail 1..14, transitions idle centre<->repos, idles persistants et affichage maisons/hotels | `SequenceRuntime`, `SequencePlayback`, GameQueueGate, PC3D moderne, BoardGeometry, RULE | Placement/orientation, pose runtime, cameras 3/5/15, `PlanMoveAnim`, faillite/victoire, interpolation Bezier, paddywagon GoToJail 1..14, transitions centre/repos et `Player3DTokenShown` sont portes/testes. Les maisons/hotels utilisent les HMD 4/5, l echelle 0.10 et les priorites historiques 55 + square*4 + slot; promotion/downgrade et masquage du plateau sont raccordes au SequenceRuntime. Restent principalement les des 2D/3D, shadows, effets audio/Pennybags et quelques aspects lifecycle/options. |
...
- reconstruction **clean-first complete** de `modern/build-phase-c`, code de
  ...
  `MonopolyGPU3DCore` ;
- suite complete depuis ce build neuf : **48/48 suites passees**, zero echec ;
- trace conservee dans le build ignore sous
  `modern/build-phase-c/ctest-full-20260906-buildings.log` ;
