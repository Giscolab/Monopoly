...
## Bilan consolide du 13 septembre 2026

Etat consolide apres `b59b9a7` sur `master`. Les runs successifs sont regroupes ci-dessous par commit
livre : un travail prepare puis integre dans un run suivant n'est compte qu'une
fois. Aucun changement de la journee ne touche `Source/`.

**Progression fonctionnelle estimee : environ 75 %**, contre 74 % au checkpoint
`18eecae` de 15:10. C'est une appreciation prudente de la maturite des contrats
portes, avec une incertitude d'environ trois points, pas une mesure exhaustive
ni un pourcentage de jeu retail jouable. Les raccordements achat/enchere,
tour/logement et dette justifient une hausse limitee d'un point; leur validation
encore incomplete ne justifie pas de declarer l'IA terminee.

Les trois indicateurs ont des perimetres differents :

- **75 % fonctionnels estimes** : appreciation du port global; aucun calcul
  automatique ni decompte de lignes ne donne cette valeur.
- **32/47 familles engagees = 68,1 %** : matrices Jeu Monopoly et ArtLib
  (hors detail PC3D et DATA);
  2 `PORTED_COMPLETE`, 3 `REPLACED_PORTABLE`, 27 `PORTED_PARTIAL`,
  15 `NOT_STARTED`. Les statuts de ces familles restent inchanges.
- **52 % d'indice automatique** : le script GitHub parcourt aussi la matrice
  Donnees et verification, soit 74 entrees actives : 13 completes, 5 remplacees,
  41 partielles et 15 non demarrees. Il donne 100 aux completes/remplacees,
  50 aux partielles et 0 aux non demarrees, puis tronque la moyenne; les
  4 entrees bloquees et l'outillage manquant sont exclus. Des sous-contrats
  recoupent les familles principales : cet indice n'est pas l'estimation globale.
...
La journee avance surtout l'IA existante; elle ne complete ni une nouvelle
famille graphique, ni l'audio, ni le reseau. Le rendu D3D12 et les acquis UI
anterieurs restent des acquis anterieurs. Le checkpoint `b59b9a7` raccorde aussi `CardSeen`, l acceptation de configuration,
le reset des transitoires de resync sans purger les profils et le nettoyage
des IA eliminees/off-board.
