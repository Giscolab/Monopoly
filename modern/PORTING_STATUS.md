...
- suite complete depuis ce build neuf : **75/75 suites passees**, zero echec ;
...
- le routage input respecte maintenant `UDAUCT -> UDBOARD -> UDIBAR -> UDPSEL`; les mouvements SDL transportent X/Y et DeltaX/DeltaY dans le repere logique 800x600, et le controle manuel souris de la camera 3D est teste de bout en bout avec release strictement apres 20 secondes ;
...
1. Passer maintenant a `UDTrade` dans l ordre source : fermer d abord les projections d etat, geometries, hit-tests et transitions autonomes qui disposent deja d owners `RuleTrade`/`LocalPlayers`/`Display`; ne pas fabriquer les surfaces texte/fonts ni la persistence absente. Le chemin autonome `UDAuct` est ferme hors surfaces texte et audio/voix Pennybags.
