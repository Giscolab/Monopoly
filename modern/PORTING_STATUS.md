...
| `Source/monopoly/Ai_util.cpp` | `AIUtility`, `BoardRules` | `PORTED_PARTIAL` | groupes/sets de proprietes, monopoles, lots, loyers, proximite, revenus/depenses, maisons/hotels, actifs liquides, valeur totale | `GameState`, `BoardRules`; futurs `AI_State`/`Ai_load` | `AIUtility` porte maintenant le noyau pur de base de `Ai_util.cpp` sans etat global AI : plages `AI_Group_Start_End`, sets/comptage, detection/proprietaire de monopoles, lots, maisons/hotels et shortage, railroads/utilities, `AI_Rent_If_Stepped_On`, table retail des frequences d atterrissage, proximite joueur/monopole, cout d unmortgage a 110 %, cash cows, revenu potentiel, rente moyenne recue/payee, depenses minimales, actifs liquides de proprietes et total worth. Les quirks retail (railroads dans `AI_Is_Monopoly`, exclusion dans `AI_Own_Monopoly`, semantique mortgage, inversion historique `count_baltic`, calcul fiscal utilise par `AI_Average_Rent_Payed`) sont preserves/testes. Restent notamment `AI_State.money_owed`, highest-rent/best-current-rent, hypothese achat/unmortgage, heuristiques chargees depuis `*.AI`, trading et boucle de decision. |
