...
| `Source/monopoly/UDPieces.cpp` | `PiecePlacement`, `PieceRuntime`, `PieceCamera`, `PieceMovePlan`, `PieceMovePlayback`, `PieceMoveIngress`, `PieceInterpolation`, `PieceJailPlan`, `PieceJailPlayback`, `PieceIdleTransition`, `PieceIdlePlayback`, `BoardGeometry` | `PORTED_PARTIAL` | orientation tokens/repos, maisons/hotels, pose runtime, cameras, TokenAnimStack, game-queue movement ingress, GoToJail 1..14, transitions idle centre<->repos | `SequenceRuntime`, `SequencePlayback`, GameQueueGate, PC3D moderne, BoardGeometry, RULE | Placement/orientation, GetInfo/GetChildMeshWorldMatrix -> LastKnownData, cameras 3/5/15, `PlanMoveAnim`, execution runtime de la stack, faillite/victoire, interpolation Bezier UDUtils, machine paddywagon GoToJail 1..14 et transitions centre/repos sont portes/testes. `NotifyStartTurn` planifie l ancien centre et le nouveau joueur avant mutation de `currentPlayer`; les deux sequences idle sont surveillees et arretees independamment avant unlock. Restent notamment idles persistants `Player3DTokenShown`, dice/show/lifecycle, shadows/buildings visuels et effets audio/Pennybags. |
...
- reconstruction **clean-first complete** de `modern/build-phase-c`, code de
  ...
  `MonopolyGPU3DCore` ;
- suite complete depuis ce build neuf : **46/46 suites passees**, zero echec ;
- trace conservee dans le build ignore sous
  `modern/build-phase-c/ctest-full-20260906-piece-idle.log` ;
