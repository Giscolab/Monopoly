...
1. Continuer `UDIBar` sur l'entree utilisateur maintenant que rendu et rectangles
   sont prouves : synchroniser le masque de slots actifs avec le playback, porter
   d'abord les clics qui envoient directement une action RULE (Buy/Auction, taxe,
   prison, trade, Bankrupt, shortage), puis les sous-etats BSSM et la selection
   de deed/`DeedActive` sans court-circuiter leur lifecycle historique.
