...
| `Source/monopoly/UDTrade.cpp` | `TradeUI`, `TradePropertyPlayback`, `UserInterface`, `IBar`, `LocalPlayers::tradeSourcePlayer`, `RuleTrade` | `PORTED_PARTIAL` | `UDTrade_GetPlayerToTradeFrom`, entree ecran Trade, selection partenaire, liste editeur, cash, deeds, jail cards, futures/immunites, notifications started/editor/item/acceptance/finished, counter-offer | miroir UI, RuleTrade, LocalPlayers, DISPLAY, IBar, SequencePlayback | Le chemin interactif est maintenant largement porte : selection partenaire, logique cash retail, cartes sortie de prison, Propose/Cancel, `Trade_SendItems`, deeds/proprietes avec hit-test source exact, projection before/offered/after, hypotheques, futures/immunites et nettoyage des contrats devenus invalides. `TradePropertyPlayback` rend les quatre ensembles statiques reellement affiches par le retail (before A/B + offered A/B) avec PAT `TAB_trprfm0000..27` / `TAB_trprfp0000..27`, priorites 324/374/424/474, positions/ordre historiques et transitions Stop/Start/Move transactionnelles. Les structures after A/B restent une projection logique : leur repositionnement 4/5 est commente dans le source retail et n est donc pas invente. Restent surtout l animation de transfert de deed `MOVESTEP=4` cadence 25 ms, les icones jail/future/immunity en playback, les surfaces/dialogues/panels et la presentation texte/fonts/audio. |
...
- suite complete depuis ce build neuf : **77/77 suites passees**, zero echec ;
...
- `UDTrade/TradeUI` couvre maintenant selection partenaire, cash, jail cards, deeds/proprietes, Propose/Cancel, `Trade_SendItems` et futures/immunites; `TradePropertyPlayback` publie les quatre boites de deeds statiques utilisees par le retail (before A/B + offered A/B), avec assets PAT normaux/hypotheques, priorites 324/374/424/474, positions/ordre sources, MoveXY lors des recompositions et rollback complet si ressource/FIFO insuffisante. Les ensembles after A/B restent logiques car leur repositionnement graphique 4/5 est commente dans `UDTrade.cpp`;
...
1. Continuer `UDTrade` dans l ordre source : porter maintenant l animation de transfert de deed `movingcard` (`MOVESTEP=4`, pas de 25 ms, priorite volante source), puis les playbacks statiques jail/future/immunity et les surfaces de dialogue/panneaux encore manquantes. Conserver texte/fonts/audio explicitement differes tant que leurs backends portables ne sont pas disponibles. Le chemin autonome `UDAuct` reste ferme hors surfaces texte et audio/voix Pennybags.
