...
| `Source/monopoly/UDPieces.cpp` | `PiecePlacement`, `PieceRuntime`, `PieceCamera`, `PieceMovePlan`, `PieceMovePlayback`, `PieceMoveIngress`, `PieceInterpolation`, `PieceJailPlan`, `PieceJailPlayback`, `BoardGeometry` | `PORTED_PARTIAL` | orientation tokens/repos, maisons/hotels, pose runtime, cameras, TokenAnimStack, game-queue movement ingress, GoToJail 1..14 | `SequenceRuntime`, `SequencePlayback`, GameQueueGate, PC3D moderne, BoardGeometry, RULE | Placement/orientation, GetInfo/GetChildMeshWorldMatrix -> LastKnownData, cameras 3/5/15, `PlanMoveAnim`, execution runtime de la stack, faillite/victoire, interpolation Bezier UDUtils et machine paddywagon GoToJail 1..14 sont portes/testes. Les notifications RULE alimentent le planner avant mutation de la projection UI; le lock est libere a la fin normale, au premier loop victoire ou au cleanup prison. Restent notamment transitions idle centre<->repos, dice/show/lifecycle, shadows/buildings visuels et effets audio/Pennybags. |
...
- reconstruction **clean-first complete** de `modern/build-phase-c`, code de
  ...
  `MonopolyGPU3DCore` ;
- suite complete depuis ce build neuf : **44/44 suites passees**, zero echec ;
- trace conservee dans le build ignore sous
  `modern/build-phase-c/ctest-full-20260906-jail-integration.log` ;
