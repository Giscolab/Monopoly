...
1. Continuer `UDTrade` dans l ordre source sur le socle `TradeUI` maintenant valide : porter ensuite les dialogues/inputs cash, mouvements de properties et jail cards, puis Propose/Cancel + emission `Trade_SendItems`; enchainer ensuite futures/immunites. Conserver texte/fonts/audio explicitement differes tant que leurs backends portables ne sont pas disponibles. Le chemin autonome `UDAuct` reste ferme hors surfaces texte et audio/voix Pennybags.
