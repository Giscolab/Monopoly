...
1. Continuer `UDBoard` dans l ordre source apres le LIGHTING maintenant porte : auditer les contrats init/show encore actifs autour de `viewportBackgroundFillOn`/`LE_REND3D_ClearBeforeRender`, `g_bOptionsButtonsOn` et des backdrops statiques PlayerSelect/Rules/Auction/Options; ne raccorder que les ressources disponibles et conserver le chemin custom 2D `city<0` differe tant que `g_aid2DBoards`/payloads externes ne sont pas disponibles.
