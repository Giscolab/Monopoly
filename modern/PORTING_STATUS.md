...
- suite complete depuis ce build neuf : **78/78 suites passees**, zero echec ;
...
1. Continuer `UDTrade` dans l ordre source : `movingcard` et les playbacks statiques jail/future/immunity sont fermes; auditer puis porter les surfaces de dialogue/panneaux encore manquantes, en commencant par les sequences DAT reellement autonomes des dialogues cash avant les surfaces runtime dependantes des fonts. Conserver texte/fonts/audio explicitement differes tant que leurs backends portables ne sont pas disponibles. Le chemin autonome `UDAuct` reste ferme hors surfaces texte et audio/voix Pennybags.
