...
1. Continuer `UDBoard` dans l ordre source apres backdrop 2D multi-ville + ownership/mortgage + cameras : auditer d abord la branche 3D dependante de `city` puis l eclairage/options et ne porter que les callers actifs; conserver le chemin custom `city<0` differe tant que `g_aid2DBoards`/payloads externes ne sont pas disponibles.
