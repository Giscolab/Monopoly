...
| `Source/monopoly/UDTrade.cpp` | `TradeUI`, `UserInterface`, `IBar`, `LocalPlayers::tradeSourcePlayer`, `RuleTrade` | `PORTED_PARTIAL` | `UDTrade_GetPlayerToTradeFrom`, entree ecran Trade, selection partenaire, liste editeur, notifications started/editor/item/acceptance/finished, counter-offer | miroir UI, RuleTrade, LocalPlayers, DISPLAY, IBar | Le bouton Trade UDIBar est maintenant fonctionnel : joueur IBar inspecte -> helper local exact -> `Screen2D::Trade`, feedback Pressed seulement si l entree reussit et preservation d un trade stocke valide. Le selecteur partenaire porte `(306,234)`, 188x209, priorite 205, token hitboxes 53x29 espacees de 32, exclusion A/off-board, clavier 1..6, raccourci deux joueurs `1-A`, cartes prison et `desiredTradePanels`; le premier MouseLeftDown d entree est avale comme `g_bFirstTime`. Les notifications projettent A/B, edit/view, cash/property/jail/future/immunity, purge d une reprise distante, acceptance et finish; le counter local inverse A/B et reecrit le sender des items. La logique d edition cash est maintenant fonctionnelle avec les hotspots retail, boutons 500/100/50/20/10/5/1, saisie decimale, Backspace, Enter/Okay, Clear, Cancel restaurant valeur et direction, plafond 999999 et recalcul des deux soldes. Les cartes sortie de prison Chance/Community Chest basculent exactement entre les quatre bits before/after et la liste TradeItem. Propose exige un give+get significatif et emet ACTION_START_TRADE_EDITING; Cancel nettoie l editeur et revient a Main. Sur NotifyTradeEditor local, `Trade_SendItems` est reproduit par un batch ordonne items -> `TradeEditingDone(0, private=1)` avec preflight de capacite FIFO qui interdit toute emission partielle. Restent surtout les mouvements/hit-tests de deeds/proprietes, les dialogues futures/immunites et la presentation texte/fonts/audio. |
...
- suite complete depuis ce build neuf : **76/76 suites passees**, zero echec ;
...
- `UDTrade/TradeUI` dispose maintenant d un chemin interactif reel : le bouton Trade UDIBar ouvre `Screen2D::Trade`, le selecteur partenaire reproduit geometrie/keyboard/two-player et le guard `g_bFirstTime`, puis `UserInterfaceRouting` prouve premier clic avale -> second clic B; `TradeUI` verrouille aussi la liste legacy 105 items, les cles de remplacement, clamp 99, `AddUiImmunity`, les notifications Started/Editor/Item/Acceptance/Finished et le counter-offer local, avec purge des items stale lors d une reprise par proposant non local. La logique cash retail est couverte de bout en bout cote input/projection/messages (billets, clavier, Clear/Cancel/Okay, direction restauree, soldes A/B), les deux cartes prison basculent before/after, Propose impose give+get, Cancel revient a Main, et `NotifyTradeEditor` emet `Trade_SendItems` puis `TradeEditingDone` seulement si la FIFO peut accepter le batch complet ;
...
1. Continuer `UDTrade` dans l ordre source sur le socle `TradeUI` maintenant valide : le cash, les jail cards, Propose/Cancel et `Trade_SendItems` sont fermes; porter maintenant les mouvements/hit-tests de deeds/proprietes et leur recalcul before/after, puis enchainer les dialogues futures/immunites. Conserver texte/fonts/audio explicitement differes tant que leurs backends portables ne sont pas disponibles. Le chemin autonome `UDAuct` reste ferme hors surfaces texte et audio/voix Pennybags.
