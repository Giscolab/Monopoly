...
1. Continuer `UDTrade` dans l ordre source : `movingcard`, les playbacks statiques jail/future/immunity, le popup cash autonome et les fleches Future/Immunity sont fermes; auditer les autres panneaux Trade actifs, mais ne pas materialiser `rightpanel`/listes/texte/Okay tant que leur backend GRAFIX/FONTS portable n existe pas. Conserver texte/fonts/audio explicitement differes tant que leurs backends portables ne sont pas disponibles. Le chemin autonome `UDAuct` reste ferme hors surfaces texte et audio/voix Pennybags.
2. Enchainer ensuite `UDOpts` avec la meme discipline de caller/owner moderne; `UDPsel` et les contrats autonomes actifs de `UDPieces` restent fermes hors persistence/fonts/audio/options externes.
