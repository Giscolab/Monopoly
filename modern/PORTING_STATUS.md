...
1. Continuer `UDBoard` dans l ordre source apres LIGHTING et le fill viewport maintenant portes : les backdrops statiques PlayerSelect/Rules/Auction/Options restent bloques par leurs DAT absents et `g_bOptionsButtonsOn` appartient a UDOpts; auditer ensuite les derniers contrats effectivement actifs de `UDBOARD_SelectAppropriateView`, preload/camera et lifecycle avant de fermer ce module, tout en conservant le chemin custom 2D `city<0` differe tant que `g_aid2DBoards`/payloads externes ne sont pas disponibles.
