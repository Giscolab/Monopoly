...
- suite complete depuis ce build neuf : **70/70 suites passees**, zero echec ;
...
- `BoardBackdropPlayback` reproduit le cache 2D UDBoard USA : 4 buffers Main 800x450, 1 buffer Portfolio/Trade 400x225, `BMP_mybs*`/`BMP_mybss*`, priorite 10, offsets viewport et eviction historique ;
...
1. Continuer `UDBoard` dans l ordre source apres backdrop + ownership/mortgage + input camera souris : auditer puis porter demo/floating camera et manual fixed-camera lock uniquement selon leurs callers actifs; terminer ensuite la composition ville/systeme du backdrop quand ses assets/callers sont prouves.
2. Revenir ensuite a `UDIBar` pour les derniers consommateurs purement sequence; conserver cash/noms comme donnees jusqu au backend minimal fonts/surfaces et ne pas inventer de rendu texte ni de SFX avant leurs services portables.
